...
--
-- Host: localhost
-- Generation Time: Jun 06, 2016 at 06:03 AM
...
--
-- Database: `codemara_povilas`
--

-- --------------------------------------------------------

--
-- Table structure for table `acon`
--

CREATE TABLE IF NOT EXISTS `acon` (
...
(102, 'fisn and chips', '7.99', 1, 0),
(101, 'bacon strips', '9.95', 1, 0),
(103, 'frog paste', 'beef stake', NULL, 66),
(104, 'chicken paste', '8.88', NULL, 66),
(105, 'beef chops', '7.69', 1, 0),
(107, 'sack of ham', '9.95', 1, 0);
...
--
CREATE TABLE IF NOT EXISTS `mostbids` (
`id` int(11)
,`name` varchar(50)
,`bids` int(11)
);
...
INSERT INTO `users1` (`id`, `name`, `address`, `city`, `state`, `zip`) VALUES
...
CREATE ALGORITHM=UNDEFINED DEFINER=`codemara`@`localhost` SQL SECURITY DEFINER VIEW `mailing` AS select concat(`users1`.`city`,',',`users1`.`state`) AS `address` from `users1`;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`codemara`@`localhost` SQL SECURITY DEFINER VIEW `mostbids` AS select `items`.`id` AS `id`,`items`.`name` AS `name`,`items`.`bids` AS `bids` from `items` order by `items`.`bids` desc limit 10;
